...
rdesplazante estructural(//estructural yosys con biblioteca de componentes
  .CLK(clk),
  .ENB(enb),
  .DIR(dir),
  .S_IN(s_in),
...
//    @(posedge clk);
    $display("--------------------------------------------------------------------------------------");
    $display("#####  FIN COMPARACION DE SINTESIS DE YOSYS Y DESCRIPCION ESTRUCTURAL (TAREA4)   ######");
    $display("--------------------------------------------------------------------------------------");
